...
**Goal:** 实现 Ch2（数据模型）、Ch3（管线机制）、Ch4（渲染模型）三个独立 Demo，将 Source → Filter → Mapper → Actor 变成肌肉记忆。
...
│   ├── Chapter2_DataModel/                 # 新建
│   │   ├── CMakeLists.txt
│   │   ├── main.cpp                       # 入口
│   │   ├── DataModelDemo.h                # 主窗口声明
│   │   └── DataModelDemo.cpp              # 管线实现
│   ├── Chapter3_Pipeline/                  # 新建
│   │   ├── CMakeLists.txt
│   │   ├── main.cpp
│   │   ├── PipelineDemo.h
│   │   └── PipelineDemo.cpp
│   └── Chapter4_Rendering/                 # 新建
│       ├── CMakeLists.txt
│       ├── main.cpp
│       ├── RenderingDemo.h
│       └── RenderingDemo.cpp
└── docs/
    └── superpowers/
        └── notes/                          # 新建
            ├── Chapter2-DataModel.md       # 笔记模板
            ├── Chapter3-Pipeline.md
            └── Chapter4-Rendering.md
```
...
**Files:**
- Create: `src/Chapter2_DataModel/CMakeLists.txt`
- Create: `src/Chapter2_DataModel/main.cpp`
- Create: `src/Chapter2_DataModel/DataModelDemo.h`
- Create: `src/Chapter2_DataModel/DataModelDemo.cpp`
...
写入 `src/Chapter2_DataModel/CMakeLists.txt`：

```cmake
add_executable(Chapter2_DataModelDemo
    main.cpp
    DataModelDemo.h
    DataModelDemo.cpp
)

target_link_libraries(Chapter2_DataModelDemo PRIVATE
    ${VTK_LIBRARIES}
    Qt6::Core
    Qt6::Gui
    Qt6::Widgets
...
set_target_properties(Chapter2_DataModelDemo PROPERTIES
    AUTOMOC ON
    AUTOUIC ON
    AUTORCC ON
)

deploy_qt_for_target(Chapter2_DataModelDemo)
```

- [ ] **Step 2: 创建 main.cpp**

写入 `src/Chapter2_DataModel/main.cpp`：
...
写入 `src/Chapter2_DataModel/DataModelDemo.h`：
...
写入 `src/Chapter2_DataModel/DataModelDemo.cpp` 的开头部分（setupUI + 构造函数 + 骨架）：
...
DataModelDemo::DataModelDemo(QWidget* parent)
    ...
    , m_infoLabel(nullptr)
{
    setWindowTitle("Chapter 2 — VTK 数据模型");
    resize(1200, 800);
...
```cmake
add_subdirectory(src/Chapter2_DataModel)
```

- [ ] **Step 6: 构建验证**

```bash
cd e:/Dev/Personal/LearnVTK && cmake --build build --target Chapter2_DataModelDemo --config Debug
```
...
```bash
git add src/Chapter2_DataModel/ CMakeLists.txt
git commit -m "feat(Ch2): add project skeleton with UI and renderer"
```

---

### Task 2: Ch2 — 实现五种数据集构造

**Files:**
- Modify: `src/Chapter2_DataModel/DataModelDemo.cpp` — 实现 5 个 build 方法和 onDatasetTypeChanged
...
```bash
cmake --build build --target Chapter2_DataModelDemo --config Debug
./app/bin/Chapter2_DataModelDemo.exe
```
...
```bash
git add src/Chapter2_DataModel/
git commit -m "feat(Ch2): implement 5 dataset type constructors and switching"
```
...
**Files:**
- Create: `docs/superpowers/notes/Chapter2-DataModel.md`

- [ ] **Step 1: 写笔记**

写入 `docs/superpowers/notes/Chapter2-DataModel.md`：
...
```bash
git add docs/superpowers/notes/Chapter2-DataModel.md
git commit -m "docs(Ch2): add data model study notes"
```
...
**Files:**
- Create: `src/Chapter3_Pipeline/CMakeLists.txt`
- Create: `src/Chapter3_Pipeline/main.cpp`
- Create: `src/Chapter3_Pipeline/PipelineDemo.h`
- Create: `src/Chapter3_Pipeline/PipelineDemo.cpp`
- Modify: `CMakeLists.txt`

- [ ] **Step 1: 创建 CMakeLists.txt**

写入 `src/Chapter3_Pipeline/CMakeLists.txt`：

```cmake
add_executable(Chapter3_PipelineDemo
    main.cpp
    PipelineDemo.h
    PipelineDemo.cpp
)

target_link_libraries(Chapter3_PipelineDemo PRIVATE
    ${VTK_LIBRARIES}
    Qt6::Core
    Qt6::Gui
    Qt6::Widgets
...
set_target_properties(Chapter3_PipelineDemo PROPERTIES
    AUTOMOC ON
    AUTOUIC ON
    AUTORCC ON
)

deploy_qt_for_target(Chapter3_PipelineDemo)
```

- [ ] **Step 2: 创建 main.cpp**

写入 `src/Chapter3_Pipeline/main.cpp`（与 Ch2 的 main.cpp 模式相同，将 `DataModelDemo` 替换为 `PipelineDemo`）。

- [ ] **Step 3: 创建 PipelineDemo.h**

写入 `src/Chapter3_Pipeline/PipelineDemo.h`：
...
```cmake
add_subdirectory(src/Chapter3_Pipeline)
```

- [ ] **Step 6: 构建验证**

```bash
cmake --build build --target Chapter3_PipelineDemo --config Debug
```
...
```bash
git add src/Chapter3_Pipeline/ CMakeLists.txt
git commit -m "feat(Ch3): add pipeline mechanism demo with ShrinkFilter"
```

---

### Task 5: Ch3 — 添加管线变化演示 + 笔记

**Files:**
- Modify: `src/Chapter3_Pipeline/PipelineDemo.cpp` — 新增一个 Filter 切换能力
- Create: `docs/superpowers/notes/Chapter3-Pipeline.md`
...
写入 `docs/superpowers/notes/Chapter3-Pipeline.md`：
...
```bash
git add src/Chapter3_Pipeline/ docs/superpowers/notes/Chapter3-Pipeline.md
git commit -m "feat(Ch3): add filter switching + pipeline study notes"
```
...
**Files:**
- Create: `src/Chapter4_Rendering/CMakeLists.txt`
- Create: `src/Chapter4_Rendering/main.cpp`
- Create: `src/Chapter4_Rendering/RenderingDemo.h`
- Create: `src/Chapter4_Rendering/RenderingDemo.cpp`
- Modify: `CMakeLists.txt`

- [ ] **Step 1: 创建完整文件**

写入 `src/Chapter4_Rendering/CMakeLists.txt`：

```cmake
add_executable(Chapter4_RenderingDemo
    main.cpp
    RenderingDemo.h
    RenderingDemo.cpp
)

target_link_libraries(Chapter4_RenderingDemo PRIVATE
    ${VTK_LIBRARIES}
    Qt6::Core
    Qt6::Gui
    Qt6::Widgets
...
set_target_properties(Chapter4_RenderingDemo PROPERTIES
    AUTOMOC ON
    AUTOUIC ON
    AUTORCC ON
)

deploy_qt_for_target(Chapter4_RenderingDemo)
```

写入 `src/Chapter4_Rendering/main.cpp`（模式同前，类名替换为 `RenderingDemo`）。

写入 `src/Chapter4_Rendering/RenderingDemo.h`：
...
写入 `src/Chapter4_Rendering/RenderingDemo.cpp`：
...
```cmake
add_subdirectory(src/Chapter4_Rendering)
```

```bash
cmake --build build --target Chapter4_RenderingDemo --config Debug
```
...
```bash
git add src/Chapter4_Rendering/ CMakeLists.txt
git commit -m "feat(Ch4): add 4-viewport rendering demo"
```

---

### Task 7: Ch4 — 添加光照切换 + 笔记

**Files:**
- Modify: `src/Chapter4_Rendering/RenderingDemo.h` — 添加光照控制成员
- Modify: `src/Chapter4_Rendering/RenderingDemo.cpp` — 添加光照切换 UI 和回调
- Create: `docs/superpowers/notes/Chapter4-Rendering.md`
...
写入 `docs/superpowers/notes/Chapter4-Rendering.md`：
...
```bash
git add src/Chapter4_Rendering/ docs/superpowers/notes/Chapter4-Rendering.md
git commit -m "feat(Ch4): add lighting switch + rendering model notes"
```
...
将 Ch2/Ch3/Ch4 的状态从 ⬜ 改为 ✅，更新"当前任务"指向阶段二。
