...
> 状态：实施中 | 日期：2026-07-11

## 学习工作流（重要——每次对话必须遵守）

**用户自己写代码。** Claude 的角色是导师/助教，不是替身：

- ✅ 讲解概念、梳理知识点、提供参考链接、Review 用户提交的代码、出题考核
- ❌ **禁止主动写实现代码。** 除非用户明确说"帮我写这段代码"
- 计划文档中的代码示例仅供用户参考学习，不应被自动执行

典型流程：用户说"开始 ChX" → Claude 给出知识指南 → 用户自己实现 → 用户贴代码/报错 → Claude Review/答疑 → 考核 → 下一章
...
### Chapter 6 — 等值面与 Marching Cubes（1-2 周）
...
│       └── notes/                    # 学习笔记（每章一份）
├── src/
│   ├── _Backup/Chapter1_BaseVTK/     # 旧自学代码（备份参考）
│   ├── Chapter1_DataModel/           # 阶段一（Ch1-3）
│   ├── Chapter2_Pipeline/
│   ├── Chapter3_Rendering/
│   ├── Chapter4_ColorMapping/        # 阶段二（Ch4-8）
│   ├── Chapter5_ClipSlice/
│   ├── Chapter6_Contour/
│   ├── Chapter7_VectorField/
│   ├── Chapter8_VolumeRendering/
│   ├── Chapter9_OpenGLBackend/       # 阶段三（Ch9-12）
│   ├── Chapter10_Shader/
│   ├── Chapter11_CustomMapper/
│   ├── Chapter12_ComputeShader/
│   ├── Chapter13_Profiling/          # 阶段四（Ch13-15）
│   ├── Chapter14_LargeData/
│   ├── Chapter15_MultiThreading/
│   └── Chapter16_LLM/               # 阶段五（Ch16）
└── cmake/
    └── DeployQt.cmake
```
...
每个 Chapter 是**完全独立**的 CMake 子项目，不依赖任何其他 Chapter：

- 每个 Chapter 自建 Qt 窗口壳（QMainWindow + QVTKOpenGLNativeWidget）
- 窗口代码保持最简：不拖拽 .ui 文件，纯代码创建控件，只添加当章需要的交互元素
- 核心管线逻辑集中在 `setupPipeline()` 方法中，前后分明
- Chapter 之间互不引用头文件、互不链接——可以单独构建、单独运行、单独演示
...
| 风险 | 影响阶段 | 缓解措施 |
| ---- | -------- | -------- |
| **OpenGL 版本不满足 4.3**（Compute Shader 要求） | 阶段三 Ch12 | 先用 glGetString(GL_VERSION) 检查；若不支持，Ch12 改为纯 CPU 粒子系统 + Shader Storage Buffer Object 理论笔记，能力补齐后再回来做 |
...
- **构建**：CMake，每章一个独立子项目
- **平台**：Windows，不要求跨平台
- **OpenGL 版本**：阶段一/二无要求；阶段三的 Ch12（Compute Shader）需要 OpenGL 4.3，其余章节 OpenGL 3.3 即可
...
| 每章自建窗口框架 | 不共享基类 | 每章独立可运行，避免耦合，专注当章学习目标 |
